...
//chart settings
let datasetId = "9b20ce12-68a0-41dd-a9cc-4dea50aba476";
...
var bar2Field = "Appreciation Return";
...
let quarterDateField = "Max Report Date";
...
//Export File Name
let exportName = "1 YR Trailing Portfolio Performance";
...
async function parseFilter(e) {
  ...
  for (let i = 0; i < datasetSpecificFilters.length; i++) {
    ...
    if (datasetSpecificFilters[i].column === quarterFilterField) {
      outputString = `'${quarterDateField}' = ${datasetSpecificFilters[i].values}`;
...
async function fetchData(filters) {
  ...
  if (filters === "none" || filters.length === 0) {
    var filter = [];
    filter.push(firstFilter);
    var maxQuarter = await getMaxQuarter();

    quarterFilter = `'${quarterFilterField}' = '${maxQuarter}'`;
